...
let voluntersSorted = volunteerCounts.sort(sortAscending)
*/
/*let voluntersSorted = volunteerCounts.sort({
    (i: Int, j: Int) -> Bool in
    return i < j
})
*/

//let voluntersSorted = volunteerCounts.sort({ i, j in i < j })
...
anotherGrowBy500() // total growth now equal to 2500
...
let newTotalProjection = projectedPopulations.reduce(0) { $0 + $1}
